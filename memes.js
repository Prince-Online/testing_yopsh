...
function fetchMemes() {
    ...
    .then(data => {
        ...
        shuffledData.forEach((row) => {
            ...
            <span class="emoji ${userReacted ? 'reacted' : ''}" data-emoji="😂" data-count="${row.reactions.laugh}" onclick="handleEmojiClick(event, '${row.id}', 'laugh')" style="color: #000;">😂 ${row.reactions.laugh}</span>&nbsp;&nbsp;
            <span class="emoji ${userReacted ? 'reacted' : ''}" data-emoji="❤️" data-count="${row.reactions.heart}" onclick="handleEmojiClick(event, '${row.id}', 'heart')" style="color: #000;">❤️ ${row.reactions.heart}</span>&nbsp;&nbsp;
            <span class="emoji ${userReacted ? 'reacted' : ''}" data-emoji="🔥" data-count="${row.reactions.fire}" onclick="handleEmojiClick(event, '${row.id}', 'fire')" style="color: #000;">🔥 ${row.reactions.fire}</span>
            </div>
            <hr class="head-hr">
            `;
